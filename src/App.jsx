...
const nicheSolutions = [
  ...
  {
    niche: "B2B (agência, advogado, arquiteto etc.)",
    name: "Verizy",
    status: "Em desenvolvimento",
  },
];
          ...
          <div className="mx-auto max-w-6xl px-6">
            ...
            <div className="mt-24">
              <motion.div
                ...
                className="mx-auto max-w-3xl text-center"
              >
                <span className="text-xs uppercase tracking-[0.3em] text-purple-300">soluções por nicho</span>
                <h3 className="mt-4 text-3xl font-semibold text-purple-100 md:text-4xl">Produtos especializados</h3>
                <p className="mt-4 text-gray-400">
                  FluxOn já está 100% desenvolvido. Flowex, Veraxy e Verizy estão em desenvolvimento.
                </p>
              </motion.div>

              <div className="mt-12 grid gap-8 md:grid-cols-2">
                {nicheSolutions.map((solution, index) => (
                  <motion.div
                    key={solution.name}
                    variants={fadeUp}
                    initial="hidden"
                    ...
                    className="relative overflow-hidden rounded-3xl border border-purple-500/20 bg-black/50 px-8 py-10"
                  >
                    <div className="absolute -right-12 -top-12 h-32 w-32 rounded-full bg-purple-500/10 blur-2xl" />
                    <div className="relative">
                      <div className="flex flex-wrap items-center justify-between gap-4">
                        <span className="text-xs uppercase tracking-[0.3em] text-purple-300">{solution.niche}</span>
                        <span className="rounded-full border border-purple-500/40 px-3 py-1 text-[0.6rem] uppercase tracking-[0.2em] text-purple-200">
                          {solution.status}
                        </span>
                      </div>
                      <h4 className="mt-5 text-2xl font-semibold text-purple-100">{solution.name}</h4>
                      <p className="mt-3 text-sm text-gray-400">
                        Solução dedicada ao nicho de {solution.niche}.
                      </p>
                    </div>
                  </motion.div>
                ))}
